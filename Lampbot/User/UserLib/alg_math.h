 ...
 * @param Address 地址
 */
inline void Math_Endian_Reverse_16(void* Address) {
 ...
 */
inline void Math_Endian_Reverse_16(void* Source, void* Destination) {
 ...
 * @param Address 地址
 */
inline void Math_Endian_Reverse_32(void* Address) {
    uint8_t* temp_address_8 = (uint8_t*)Address;
 ...
 * @param Destination 目标存储地址
 */
inline void Math_Endian_Reverse_32(void* Source, void* Destination) {
    uint8_t *temp_source, *temp_destination;
    temp_source = (uint8_t*)Source;
    temp_destination = (uint8_t*)Destination;
 ...
 * @return uint8_t 结果
 */
inline uint8_t Math_Sum_8(uint8_t* Address, uint32_t Length) {
    uint8_t sum = 0;
 ...
 * @return uint16_t 结果
 */
inline uint16_t Math_Sum_16(uint16_t* Address, uint32_t Length) {
    uint16_t sum = 0;
    for (int i = 0; i < Length; i++) {
        sum += Address[i];
    }
 ...
 * @return uint32_t 结果
 */
inline uint32_t Math_Sum_32(uint32_t* Address, uint32_t Length) {
    uint32_t sum = 0;
    for (int i = 0; i < Length; i++) {
        sum += Address[i];
    }
 ...
 * @return float 输出
 */
inline float Math_Sinc(float x) {
    //分母为0则按极限求法
